...
function getRecentLogs(n, time) {
  var cmd = ""
  if (time) {
    cmd = "find -mmin -"+time+" -type f -printf '%T@ %p\\0' -maxdepth 3 2> /dev/null | sort -zk 1nr | sed -z 's/^[^ ]* //' | tr '\\0' '\\n' | grep -E '\\./[^.]+\\.log$' | head -n "+n;
  } else {
    cmd = "find -type f -printf '%T@ %p\\0' -maxdepth 3 2> /dev/null | sort -zk 1nr | sed -z 's/^[^ ]* //' | tr '\\0' '\\n' | grep -E '\\./[^.]+\\.log$' | head -n "+n;
  }
  var logfile = execute(cmd);
